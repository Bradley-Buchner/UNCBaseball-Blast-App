...
best.swings <- function(name, metric, date) {
  ...
  df <- data %>% filter(Date...2 == date) %>% select(`Plane Score`, `Connection Score`, `Rotation Score`,`Bat Speed (mph)`, `Peak Hand Speed (mph)`, `Time to Contact (sec)`)
...
worst.swings <- function(name, metric, date) {
